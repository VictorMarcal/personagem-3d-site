...
// introdução - só é possível alterar a cada 15 dias" + "gráfico que mostra a
// evolução do peso").
//
...
//
// A REGRA DOS 15 DIAS E VALIDADA CONTRA O SERVIDOR, nao contra o
...
const WEIGHT_MIN_DAYS_BETWEEN = 15;
...
// Devolve quantos dias faltam para se poder voltar a alterar. 0 = ja pode.
function diasAteProximaAlteracao() {
  const ultimo = ultimoRegistoPeso();
  if (!ultimo) return 0;
  const passados = (Date.now() - ultimo.data.getTime()) / MS_POR_DIA;
  return Math.max(0, Math.ceil(WEIGHT_MIN_DAYS_BETWEEN - passados));
}
...
async function registarPeso(kg) {
  ...
  const faltam = diasAteProximaAlteracao();
  if (faltam > 0) {
    return {
      ok: false,
      motivo: `Só podes alterar o peso a cada ${WEIGHT_MIN_DAYS_BETWEEN} dias. Faltam ${faltam} ${faltam === 1 ? "dia" : "dias"}.`,
    };
  }
...
function renderWeightHistory() {
  ...
  if (faltam > 0) {
    estado.textContent = `Último registo: ${formatKg(ultimo.kg)} em ${ultimo.data.toLocaleDateString("pt-PT")}. Podes voltar a alterar daqui a ${faltam} ${faltam === 1 ? "dia" : "dias"}.`;
    ...
    estado.textContent = ultimo
      ...
      : `Podes registar o teu peso. Depois disso, só a cada ${WEIGHT_MIN_DAYS_BETWEEN} dias.`;
