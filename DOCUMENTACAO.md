...
- **Regra 1 (não é possível melhorar uma peça acima do nível atual do jogador)**: herdada do sistema anterior — `upgrade()` (`createEquipmentUpgradeController`, `js/equipment.js`) recusa subir a peça para um nível de melhoria acima de `getCurrentPlayerLevel()`, mesmo havendo moedas suficientes. O popup mostra "Sobe de nível de personagem para continuar a evoluir esta peça" nesse caso, em vez do botão de evoluir
