...
- **Sem limite pelo nível de personagem**: a regra herdada do sistema anterior (não podia melhorar-se uma peça acima do nível de personagem atual) existiu só por um dia e foi removida a pedido em 2026-08-05 — a única condição para evoluir uma peça é ter materiais suficientes, até ao nível 99
- **Popup de evolução** (botões da mini-lista `#equipment-mini-weapon/shield/armor`, secção 12 — deixou de se poder abrir clicando no modelo 3D, ver "Placeholder removido" na secção 9): mostra o nível atual (`Lv/99`), estatísticas atuais e do próximo nível (com a diferença), o custo em materiais e o stock atual desses materiais (`#<peça>-upgrade-cost`/`-stock`) e um botão para evoluir — sem inventário nenhum, já que só existe uma peça por tipo. As 3 peças usam o mesmo popup genérico (`createEquipmentUpgradeController(config)`, `js/equipment.js`), instanciado uma vez por peça (`weaponUpgradeController`/`shieldUpgradeController`/`armorUpgradeController`)
