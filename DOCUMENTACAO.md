...
**Dificuldade só por cor (2026-09-20, a pedido — "quero que tenham apenas cores: verde = fácil, amarelo = médio, vermelho = difícil, não quero muito contraste")**: os cartões deixaram de ter o chip com o nome da dificuldade (o título da secção já o diz). O cartão leva a classe `mission-card-facil|media|dificil` (`renderMissoesAtivas`/`renderMissionSlotBlock`, `js/missions.js`) com um fundo/contorno suave — `#ebf1df`/`#f6eed3`/`#f4e0d8` (`css/campo-aberto-v6.css`), de luminosidade parecida para não haver um a sobrepor-se. A linha do cartão fica só com a recompensa, alinhada à direita.
