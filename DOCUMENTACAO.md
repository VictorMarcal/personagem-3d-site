...
| `assets/Floor.glb` | Terreno/cenário da aba Eu › Personagem (secção 9), `loadSceneryFloor()` em `js/main.js` — WIP |
...
- **Alcance** (substituiu Destreza, 2026-09-16): distância a partir da qual a personagem começa a disparar sozinha — sem efeito na arena por agora (usa frustrum/joystick, não distância), usado pela horda (secção 23) para saber quando um monstro está perto o suficiente. **Efeito visual** (2026-09-16, a pedido): um anel azul semitransparente no chão da aba Eu › Personagem, centrado na base da torre, com o raio do Alcance atual (`attackRangeRing`/`updateAttackRangeRing()`, `js/main.js`) — recalculado sempre que `renderStatsHud()` corre (`js/equipment.js`), escondido durante uma luta na Masmorra (`enterBattleView`/`exitBattleView`, tal como os monstros de uma horda). A altura do anel segue o relevo real do terreno (raycast a partir de cima na origem, feito uma vez quando o Floor.glb carrega), para não ficar enterrado nem a flutuar num terreno que não é plano.
...
Enquanto uma horda decorre, a câmara muda para `CameraAtackPosition` (mais alta/recuada, vê-se o terreno todo) em vez da vista normal — `applyHordaCamera()`/`applyNormalCamera()`, chamadas em `iniciarHorda()`/`terminarHorda()`. `applyPersonagemCamera()` escolhe qual aplicar sempre que a câmara volta a ser posta na cena Eu › Personagem por outro motivo (fim de luta na Masmorra, modelos a carregar pela primeira vez), para uma horda em curso não perder a vista de topo ao entrar/sair da Masmorra.
