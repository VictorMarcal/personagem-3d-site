...
- **Popup de troféus de outro jogador** (2026-08-04, clicar no nome de qualquer linha nas abas Geral/Mensal): mostra a grelha de conquistas dele, agrupada por categoria — a mesma grelha/estilo do popup "Ver todas" (secção 10), só que a partir do `unlocked_achievements` **desse jogador** em vez do próprio. `player_progress` é privado (RLS só deixa o dono ler a sua própria linha), por isso uma **cópia pública** de `unlocked_achievements` foi adicionada à tabela `leaderboard` (já a projeção pública intencional, sincronizada em `syncProgressToSupabase()`/`migrateLocalProgressToSupabase()` a par do resto). As funções de `js/achievements.js` (`isAchievementUnlocked`, `findWonMedalForMonthNumber`, `generateMonthlyMedalAchievements`, `getAllAchievements`, `createAchievementItemEl`) passaram a aceitar um `unlockedMap`/`onClick` opcionais (default = o próprio jogador/popup de detalhe existente, comportamento inalterado) para serem reutilizadas aqui sem duplicar lógica. Os itens de outro jogador ficam **sem clique** (classe `read-only`) — não há dados de progresso/data de desbloqueio de outro jogador para mostrar num popup de detalhe, só desbloqueada/por desbloquear
