...
```
peça.primário(Lv)   = base + round(Lv ^ EQUIP_PRIMARY_EXPONENT)
...
EQUIP_PRIMARY_EXPONENT = 1.2, EQUIP_SECONDARY_EXPONENT = 0.5, EQUIP_SECONDARY_MAX = 20
...
- **Dano flutuante** (`showFloatingCombatText(head, valor, variant)`, `js/main.js`): cada acerto mostra um número a subir e desvanecer por cima da cabeça de quem foi atingido, projetado a partir da posição 3D real da cabeça (`head`/`monsterHead`) para o ecrã — só precisa de projetar uma vez por acerto, já que a câmara e as posições ficam fixas durante toda a luta. `variant` (2026-08-05) escolhe a cor via CSS (`.floating-combat-text.<variant>`) e o texto mostrado: dano normal em **amarelo** (`"damage"`), crítico em **vermelho** (`"critico"`, distinto do dano normal - antes ambos eram vermelhos), esquiva mostra **"Miss"** em vez de um número (`"miss"`, amount sempre 0) — cura fora de combate continua em verde (`"heal"`, default quando `variant` não é passado e `amount > 0`, ver secção 7). Um pequeno deslocamento aleatório (`FLOATING_COMBAT_TEXT_JITTER_PX = 20`px em X e Y) evita que vários números seguidos no mesmo alvo fiquem todos exatamente empilhados na mesma posição
