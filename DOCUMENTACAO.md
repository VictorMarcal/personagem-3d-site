...
- **Popup de troféus de outro jogador** (2026-08-04, clicar no nome de qualquer linha nas abas Geral/Mensal, exceto a **própria** — não faz sentido abrir isto no seu próprio nome, já vê os seus troféus no Jogo/Perfil): mostra **só as conquistas já desbloqueadas** dele, agrupadas por categoria (nunca as por desbloquear — pedido explícito, é uma lista de troféus reais, não um checklist de progresso de outro jogador; mostra "Ainda sem troféus." se a lista vier vazia). Reaproveita o agrupamento/desenho de `js/achievements.js`, a partir do `unlocked_achievements` **desse jogador** em vez do próprio. `player_progress` é privado (RLS só deixa o dono ler a sua própria linha), por isso uma **cópia pública** de `unlocked_achievements` foi adicionada à tabela `leaderboard` (já a projeção pública intencional, sincronizada em `syncProgressToSupabase()`/`migrateLocalProgressToSupabase()` a par do resto). As funções de `js/achievements.js` (`isAchievementUnlocked`, `findWonMedalForMonthNumber`, `generateMonthlyMedalAchievements`, `getAllAchievements`, `createAchievementItemEl`) passaram a aceitar um `unlockedMap`/`onClick` opcionais (default = o próprio jogador/popup de detalhe existente, comportamento inalterado) para serem reutilizadas aqui sem duplicar lógica. Os itens ficam **sem clique** (classe `read-only`) — não há dados de progresso/data de desbloqueio de outro jogador para mostrar num popup de detalhe
