...
**Sistema de moedas — REMOVIDO (2026-09-08, v5.4.0)**: existiu de 2026-08-04 a 2026-09-08. Oferta inicial de 100, ganhas a treinar (50% de chance por km real de encontrar 1-20), a derrotar mini-bosses/bosses (`awardMonsterCoins`) e a desbloquear conquistas (tabela `ACHIEVEMENT_COIN_REWARD`); gastas a evoluir o equipamento. Quando o equipamento passou a custar **materiais do mapa** (secção 21, v5.0.0), as moedas ficaram sem uso — ganhas mas gastas em nada — e foram removidas de vez: HUD (`.header-coin-pill`), cartão de moeda encontrada no treino, `getMoedas/addMoedas/spendMoedas`, `awardMonsterCoins`, `ACHIEVEMENT_COIN_REWARD`/`MONTHLY_MEDAL_COIN_REWARD`, as 6 conquistas de moedas (`coins_earned_*`, `spender_*`) e as colunas `player_progress.moedas/total_moedas_ganhas/total_moedas_gastas` (`drop column`, secção 14). Nível/XP em calorias, pontos de status, materiais e medalhas mensais não foram tocados. História completa em git.
...
- `coinsEarned` / `coinsSpent` — **REMOVIDAS (2026-09-08)** junto com as moedas (secção 7). Eram `coins_earned_500/2000/10000` e `spender_500/2000/10000`, ligadas a `total_moedas_ganhas`/`total_moedas_gastas` (colunas entretanto `drop`)
...
- ~~O saldo de moedas de cada jogador não foi convertido nem apagado.~~ As moedas foram removidas de vez em 2026-09-08 (secção 7): colunas `moedas`/`total_moedas_ganhas`/`total_moedas_gastas` com `drop column`.
