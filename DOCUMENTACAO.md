...
- **Anúncio público na landing** (`#landing-beta-banner`, `mostrarAnuncioDeBeta()` em `js/auth.js`, chamado sem precisar de sessão): a MESMA `beta_vagas_restantes()` acima, só para mostrar "Beta fechada — N vagas disponíveis para experimentar" (singular/plural tratado) ou "sem vagas disponíveis" a quem chega à página, antes mesmo de tentar criar conta. Escondido por omissão até a resposta chegar (nunca mostra um número errado por um instante); uma falha de rede fica em silêncio, sem banner nenhum — a landing funciona à mesma sem ele.
