...
**Tabela de 10 níveis por peça** (Arma desde 2026-08-04, Escudo/Armadura estendidas ao mesmo modelo no mesmo dia — `WEAPON_TIERS`/`SHIELD_TIERS`/`ARMOR_TIERS` em `js/equipment.js`, substituem os antigos `EQUIP_BASICO_VIDA/DEFESA`/`EQUIP_BONUS_DESTREZA/REGENERACAO` fixos): as 3 peças têm 10 tiers cada, um por nível de referência (1, 10, 20, ... 90 — passos de 10, alinhados com os bosses da secção 8). Cada tier tem os seus próprios valores de status primário, bónus secundário e custo em moedas, um conjunto por **nível de melhoria** (Lv1 a Lv9 — a mesma peça pode ainda ser melhorada dentro do seu tier, gastando moedas — ver "Sistema de moedas" abaixo):
...
- **O nível de melhoria de cada peça é guardado por tier, para sempre, e por tipo de equipamento** (`getWeaponUpgradeLevelsMap()`/`getShieldUpgradeLevelsMap()`/`getArmorUpgradeLevelsMap()`, cada um um mapa `{ tierIndex: nível }`, `player_progress.nivel_melhoria_armas/escudos/armaduras` no Supabase) — decisão explícita: encontrar/equipar a peça seguinte **não** apaga o investimento feito na anterior, mesmo que deixe de ser usada. Todos os getters/setters de melhoria/custo são genéricos (`getEquipUpgradeLevelsMap`, `getEquipUpgradeLevel`, `setEquipUpgradeLevel`, `getEquipUpgradeCost`), com finas funções por tipo (`getWeaponUpgradeLevel`, `getShieldUpgradeLevel`, etc.) só para preservar os nomes já usados no resto do código
...
**Posse e drop de equipamento** (2026-08-05 — substitui por completo o desbloqueio automático por nível que existia até então): cada tier só passa a "possuído" ao ser encontrado num drop (treino/mini-boss/boss, ver abaixo) — deixou de haver qualquer desbloqueio automático por nível de personagem. Só o tier de nível 1 de cada peça começa possuído e equipado por omissão (`getOwnedTiers`/`isTierOwned`/`addOwnedTier`, array de índices de tier por peça, `player_progress.tiers_possuidos_armas/escudos/armaduras` no Supabase, default `[0]`).

- **Equipar** (`getCurrentWeaponTierIndex()`/`getCurrentShieldTierIndex()`/`getCurrentArmorTierIndex()`, agora finas funções sobre o genérico `getEquippedTierIndex(equippedKey, ownedKey)` — já não dependem do nível de personagem, só da posse): de todas as peças já possuídas de um tipo, só uma fica "equipada" de cada vez, escolhida pelo jogador no popup (`equipWeaponTier`/`equipShieldTier`/`equipArmorTier`, `player_progress.tier_equipado_arma/escudo/armadura`, default 0). **Regra 1: não é possível equipar uma peça com `unlockLevel` acima do nível atual do jogador** (`setEquippedTierIndex` valida posse E nível antes de gravar, devolve `false` sem efeito se alguma condição falhar) — o jogador pode encontrar/guardar uma peça de nível mais alto antes de lá chegar, só não a pode usar já
- **Sorteio de um drop** (`rollEquipmentDrop(chancePercent)`, `js/equipment.js`): testa primeiro a percentagem (5% por km de treino, 7% mini-boss, 10% boss — ver "Sistema de moedas" abaixo, mesmas 3 fontes das moedas); se acontecer, escolhe ao acaso **um** dos 3 tipos de peça e depois **um** tier ao acaso dentro do intervalo elegível — `unlockLevel` a **±15 do nível atual do jogador** (`getEligibleTierIndexesForDrop`, ex: nível 2 pode dropar o tier de nível 10; nível 6 pode dropar os tiers de nível 10 ou 20). No máximo uma peça por evento, nunca as 3 ao mesmo tempo. Se o tier sorteado já for possuído, converte-se num prémio de moedas em vez de não fazer nada (`getDuplicateDropCoinReward` — média dos custos de melhoria desse tier, excluindo o passo 0 que é sempre 0; reaproveita a economia já calibrada em vez de inventar um novo valor de equilíbrio); se for novo, fica na posse do jogador (`addOwnedTier`) mas **não** se equipa automaticamente. Um aviso não-bloqueante (`showGameToast(msg, "equipamento")`, 3.5s, topo do ecrã — ver "Avisos de progresso" mais abaixo) informa o resultado (`describeEquipmentDrop`)
- **Popup de evolução** (clicar na espada/escudo/corpo no modelo 3D, `js/main.js` `raycastEquipmentAt`): mostra o tier EQUIPADO (estatísticas atuais/próximo nível de melhoria/custo em moedas para evoluir, igual a antes), mais uma secção de **inventário** por baixo — lista as 10 posições do tipo, cada uma "Equipado" (verde), "Equipar" (botão, só se possuída e `unlockLevel` <= nível atual), "Nível N necessário" (possuída mas ainda não elegível), ou "Por encontrar" (ainda não dropada). As 3 peças usam o mesmo popup genérico, gerado por uma fábrica só (`createEquipmentUpgradeController(config)`, `js/equipment.js`) instanciada uma vez por peça (`weaponUpgradeController`/`shieldUpgradeController`/`armorUpgradeController`) — mesmo HTML/CSS (`.equip-upgrade-*`/`.equip-inventory-*`), só os IDs/textos/ícone mudam por peça
- **Migração de contas existentes**: como os 2 jogadores reais estavam ambos no nível 2 no momento desta mudança (2026-08-05), o valor por omissão das novas colunas (`[0]`/`0`) já reflete corretamente o que tinham direito a possuir sob o sistema antigo (só o tier de nível 1) — não foi preciso nenhuma migração/backfill manual de dados
...
**Avisos de progresso** (`showGameToast(mensagem, variant)`, `js/equipment.js`, 2026-08-05): aviso não-bloqueante genérico, topo do ecrã, desaparece sozinho ao fim de 3.5s (`.game-toast` + `.game-toast-<variant>` no CSS, uma cor por variante) — mesmo mecanismo já usado para o drop de equipamento (variant `"equipamento"`, ver acima), estendido a mais 3 eventos de progresso:

- **Moedas** (variant `"moedas"`): a cada ganho de moedas por treino (`rollCoinDropsForKm`), mini-boss/boss (`awardMonsterCoins`) ou conquista (`unlockAchievement`, texto inclui o nome da conquista via `getAchievementName(id)`) — exceto quando a moeda vem de um drop de equipamento repetido, que já tem o seu próprio aviso combinado (ver "Sorteio de um drop" acima, evita duas mensagens empilhadas para o mesmo evento)
