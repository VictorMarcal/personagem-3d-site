...
- **Torre / Fortaleza, com o herói no topo** (`assets/Tower.glb`, 2026-09-09): `loadTower()` (`js/main.js`), mesmo padrão do terreno — base em Y=0, centrada em X/Z (o herói roda sobre a origem, tem de ser o eixo da torre), sem reescalar (~3,4×4×3,4 unidades no ficheiro atual). `TOWER_TOP_Y` guarda a altura do topo; quando a torre carrega, `character.position.y` passa de 0 para `TOWER_TOP_Y` (o herói assenta no topo) e `applyNormalCamera()` sobe a câmara e o alvo com ele — `applyNormalCamera` centra o alvo a meia altura do herói (`character.position.y + 0.9`), a câmara 0,6 acima e o `z` afastado por `TOWER_TOP_Y × 0,45` para caber a torre. Com `TOWER_TOP_Y = 0` (torre não carregou) dá exatamente a câmara `(0, 1.5, 4)`/`lookAt(0, 1, 0)` de antes. **Escondida durante uma luta** (`enterBattleView` — a arena é ao nível do chão, o herói volta a Y=0; `exitBattleView` repõe `character.position.y = TOWER_TOP_Y` e a torre). *WIP: modelo placeholder sem textura; enquadramento da câmara é um primeiro palpite.*
  ...
  - **Upgrade visual a cada 5 níveis de Fortaleza (2026-09-09, preparado)**: `towerIndexForLevel(nível) = floor(nível / 5) + 1` — nível 1-4 → `Towers/Tower1.glb`, 5-9 → `Tower2.glb`, 10-14 → `Tower3.glb`, … `refreshTowerModel()` (`js/main.js`) troca o modelo quando o índice muda; chamada no arranque, no login (o nível vem do servidor) e depois de `upgradeWarehouse()` (`js/resources-ui.js`). `applyTowerModel()` tira o anterior da cena, reassenta a base em Y=0, atualiza `TOWER_TOP_Y` e re-põe o herói no topo. **`TOWER_MODEL_COUNT`** = quantos ficheiros `Towers/TowerN.glb` já existem (a subir à medida que são feitos); enquanto for 0 usa-se o `assets/Tower.glb` único; um jogador acima do último modelo feito fica com esse. Sem HEAD-checks nem 404s. O **custo** de cada evolução (`warehouseUpgradeCost`, secção 21) é afinado à parte, para bater com o que o modelo mostra.
