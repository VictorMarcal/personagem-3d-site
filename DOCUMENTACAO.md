...
- **Bucket `feedback-screenshots`** (Storage, privado — `public: false`): a imagem sobe para `<user_id>/<timestamp>.<extensão>` antes do insert; política de `insert` exige que o 1º segmento do caminho seja o `auth.uid()` de quem envia. Uma imagem que falhe a enviar não bloqueia o texto — o feedback segue sem `screenshot_path`, avisado no toast de confirmação.
- **Não há automação a passar isto para o Trello.** Como pedido no cartão original, os tickets ficam em `feedback_reports` até o Victor pedir para serem vistos — nessa altura lê-se a tabela, cria-se um cartão por ticket na coluna **Feedback** do Trello, e apagam-se as linhas já processadas. Sem cron nem trigger: é uma leitura pontual, a pedido.
