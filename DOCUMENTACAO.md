...
**Limite de contas da beta fechada (2026-09-24, a pedido — *"quero colocar um limite máximo de inscrições"*)**: pensado para o tamanho de suporte de UM programador a corrigir dados à mão (ver secção 14.1 e os incidentes de calorias/leaderboard), não por limite técnico do Supabase gratuito (ver análise de capacidade abaixo) — **20 contas**, fácil de mudar. Duas camadas:

- **Fronteira real, no `handle_new_user()`** (trigger `on_auth_user_created`, `AFTER INSERT ON auth.users`, migração `add_beta_signup_cap`): conta as linhas de `profiles` e `raise exception` se já forem ≥ 20, **antes** de inserir o novo perfil — a exceção aborta a transação inteira, por isso a própria conta em `auth.users` nunca chega a existir. Não dá para contornar chamando a API diretamente (a mesma filosofia de "a fronteira real são as políticas RLS/trigger, nunca só a UI", já usada no resto da app).
- **Aviso amigável do lado do cliente, `js/auth.js`**: `profiles` é privado (RLS só deixa cada um ler a própria linha), por isso existe uma função à parte, `beta_vagas_disponiveis()` (SQL, `security definer`, só devolve um booleano, sem expor a tabela), chamada por `supabaseClient.rpc(...)` mesmo antes de `signUp()` no modo "Criar conta". Sem vagas, mostra "As inscrições da beta estão fechadas por agora — sem vagas." sem sequer tentar criar a conta. Se duas pessoas carregarem em "Criar conta" ao mesmo tempo para a última vaga, uma delas ainda pode acertar na verificação prévia e perder a corrida a seguir no trigger — `translateAuthError()` trata também a mensagem genérica que o Supabase Auth devolve nesse caso (`"Database error saving new user"`), com a mesma frase amigável.
