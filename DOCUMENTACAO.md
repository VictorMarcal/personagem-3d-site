...
### 14.3 Sessão única por dispositivo (2026-09-17)

A pedido (Trello, card #26 — *"A mesma conta não pode estar logada em mais que um dispositivo"*), com a UI especificada pelo próprio Victor num comentário do card. Antes disto, `js/tab-lock.js` já impedia duas **abas do mesmo telemóvel** de terem um treino/luta ativos ao mesmo tempo — mas nada impedia a mesma conta de entrar em **dois telemóveis diferentes** simultaneamente.

- **Colunas novas em `profiles`**: `active_device_id` (text) e `active_session_started_at` (timestamptz) — migração `add_active_device_session_to_profiles`, aplicada diretamente na produção (aprovado pelo Victor antes de correr). Cobertas pela política RLS `profiles_update_own` já existente, sem precisar de nenhuma política nova.
- **`DEVICE_ID`** (`js/auth.js`): um UUID gerado uma vez por dispositivo/browser e guardado em `localStorage` (`STORAGE_KEY_DEVICE_ID`, `js/storage-keys.js`) — ao contrário do `TAB_ID` de `js/tab-lock.js` (por aba, `sessionStorage`), este tem de sobreviver a fechar o browser, identifica o aparelho, não a aba.
- **No login** (`onAuthStateChange`, evento `SIGNED_IN`), antes de `bootstrapAfterLogin()`: `resolveDeviceConflict(userId)` lê `profiles.active_device_id`.
  - Vazio, erro de rede, ou já é este dispositivo → segue em frente sem popup nenhum, `claimActiveDevice()` grava `active_device_id`/`active_session_started_at` deste dispositivo.
  - Outro dispositivo diferente → mostra `#device-conflict-modal` com a especificação exata pedida: aviso de que a conta está noutro dispositivo e que continuar desliga esse outro, botões **Cancelar** (`signOut()` + reload, volta ao ecrã de login sem tocar em mais nada) e **Continuar** (`claimActiveDevice()`, entra normalmente).
- **O dispositivo anterior é desligado em tempo real**, sem precisar de um refresh manual: `subscribeToDeviceConflicts()` abre um canal Supabase Realtime (`postgres_changes`, `UPDATE` em `profiles`, filtro `id=eq.<userId>`) assim que um dispositivo reclama a sessão. Se `active_device_id` mudar para um valor diferente do seu próprio `DEVICE_ID`, esse dispositivo faz `signOut()` e recarrega a página sozinho. Exige `alter publication supabase_realtime add table public.profiles` (feito na mesma migração) — sem isto o canal nunca recebe eventos.
- **Repor palavra-passe não mostra o popup de conflito**: quem chega a esse fluxo já provou acesso ao email da conta (clicou no link de recuperação), por isso `claimActiveDevice()` é chamado direto, sem perguntar.
- **Limite conhecido**: um dispositivo que perca a ligação à internet não recebe o evento Realtime imediatamente — só ao reconectar (ou no próximo login) é que descobre que foi substituído. Aceite como suficiente para o caso de uso (evitar duplo progresso simultâneo), não é um mecanismo de segurança contra uso indevido de credenciais.
